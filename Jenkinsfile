         ...
         stage ('Deployments'){
            parallel{
                stage ('Deploy to Staging'){
                    steps {
                        bat "scp -i ${params.Senthil_M} **/target/*.war root@${params.tomcat_dev}:/var/lib/tomcat/webapps"
                    }
                }

                stage ("Deploy to Production"){
                    steps {
                        bat "scp  -i ${params.Senthil_M} **/target/*.war root@${params.tomcat_prod}:/var/lib/tomcat/webapps"
                    }
                }
            }
        }
