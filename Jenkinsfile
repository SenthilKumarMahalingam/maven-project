        ...
        stage ('Deployments'){
            parallel{
                stage ('Deploy to Staging'){
                    steps {
                        bat "ssh -i ${params.Senthil_M} **/target/*.war ec2-user@3.85.148.209:/var/lib/tomcat7/webapps"
                    }
                }
 
                stage ("Deploy to Production"){
                    steps {
                        bat "ssh -i ${params.Senthil_M} **/target/*.war ec2-user@35.168.2.91:/var/lib/tomcat7/webapps"
                    }
                }
            }
        }
